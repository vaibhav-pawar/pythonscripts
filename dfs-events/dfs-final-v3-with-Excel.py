...
def main():
    ...
    for network in networks:
        ...
        try:
            event = dashboard.networks.getNetworkEvents(networkId=net_id,productType="wireless",includedEventTypes="dfs_event",perPage=200)
    ...
    #time.sleep(1200)
    print('\n\U000027A1 Wait for next 24 Hours')
    now = datetime.now()
    ...
    for network in networks:
        ...
        try:
            event1 = dashboard.networks.getNetworkEvents(networkId=net_id,productType="wireless",includedEventTypes="dfs_event",perPage=200)
    ...
    if event_list3:
        ...
        excel_file = xlsxwriter.Workbook(f'{datetime.now().strftime("%d-%m-%Y-%H%M")}-DFSAlerts[0-Active].xlsx')
        excel_sheet = excel_file.add_worksheet("DFS Alert is Detected")
        cell_format = excel_file.add_format()
        cell_format.set_bold()
        cell_format.set_bg_color('#00A300')
        # Write header of CSV
        header = ["Network Name", "Description", "Occured At", "Device Name", "Channel", "Radio"]
        for item in header:
            excel_sheet.write(row, column, item, cell_format)
            column +=1
        row = 1
        column = 0
        for port in event_list3:
            for data in port:
        ...
        f"""<html>
            <body>
                <h5>Dear team,</h5>
                <h5>New Alert is generated on following Networks [Note: Mentioned Timezone is GMT]</h5>
                <pre>{APname}</pre>
                <h5>Thank you</h5>
            </body>
        </html>"""
